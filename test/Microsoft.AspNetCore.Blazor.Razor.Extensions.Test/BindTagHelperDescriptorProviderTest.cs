...
"));
            ...
            Assert.Equal("string Test.BindAttributes.Format", attribute.DisplayName);
...
"));
        ...
        public void Excecute_BindFallback_CreatesDescriptor()
        {
            ...
            Assert.False(bind.Metadata.ContainsKey(BlazorMetadata.Bind.ChangeHandlerAttribute));
            Assert.True(bind.IsFallbackBindTagHelper());

            Assert.Equal(
                "Binds the provided expression to an attribute and a change handler, based on the naming of " +
                    "the bind attribute. For example: <code>bind-value-changed=\"...\"</code> will assign the " +
                    "current value of the expression to the 'value' attribute, and assign a delegate that attempts " +
                    "to set the value to the 'changed' attribute.",
                bind.Documentation);
