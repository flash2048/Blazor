...
namespace Microsoft.AspNetCore.Blazor.Razor
{
    internal class BindLoweringPass : IntermediateNodePassBase, IRazorOptimizationPass
    {
        ...
        private void RewriteUsage(TagHelperIntermediateNode node, int index, ComponentAttributeExtensionNode attributeNode)
        {
            ...
            var valueNode = new ComponentAttributeExtensionNode(attributeNode)
            {
            ...
            var changeHandlerNode = new ComponentAttributeExtensionNode(attributeNode)
            {
                ...
                TagHelper = changeHandlerAttribute == null ? null : attributeNode.TagHelper,
            };
            ...
            // For now, the way this is done isn't debuggable. But since the expression
            // passed here must be an LValue, it's probably not important.
            string changeHandlerContent = null;
            if (changeHandlerNode.BoundAttribute == null && format == null)
            {
                changeHandlerContent = $"{BlazorApi.BindMethods.SetValueHandler}(__value => {originalContent} = __value, {originalContent})";
            }
            else if (changeHandlerNode.BoundAttribute == null && format != null)
            {
                changeHandlerContent = $"{BlazorApi.BindMethods.SetValueHandler}(__value => {originalContent} = __value, {originalContent}, {format})";
            }
            else
            {
                changeHandlerContent = $"__value => {originalContent} = __value";
            }
            ...
            out BoundAttributeDescriptor changeHandlerAttribute)
        {
            ...
            changeHandlerAttributeName = bindTagHelper.GetChangeHandlerAttributeName() ?? changeHandlerAttributeName;
